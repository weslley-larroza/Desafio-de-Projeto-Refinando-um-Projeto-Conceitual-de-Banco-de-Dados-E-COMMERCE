...
🛒 E-Commerce ERP: Gestão de Inventário e Vendas Este projeto apresenta um modelo de banco de dados relacional (EER) refinado para um ecossistema de e-commerce completo. A arquitetura foi projetada para ir além do simples registro de pedidos, integrando fluxos de ERP (Enterprise Resource Planning) para controle rigoroso de estoque e faturamento diferenciado.

🔹 Destaques da Implementação: Especialização Disjunta (PF/PJ): Estrutura de herança que garante a separação total entre perfis de Pessoa Física e Jurídica, impedindo a sobreposição de dados fiscais na mesma conta.
...
🛠️ Tecnologias Utilizadas: Modelo: EER (Entidade-Relacionamento Estendido)
